 ...
 */
export function extractKeywords(grammarText: string, digest: string): string[] {
    const cached = keywordCache.get(digest);
    if (cached) {
        return cached;
    }
    const factory = grammarServices.shared.workspace.LangiumDocumentFactory;
    const document = factory.fromString<GrammarAST.Grammar>(
        grammarText,
        URI.parse(`memory:/inspect-${inspectCounter++}.langium`),
    );
